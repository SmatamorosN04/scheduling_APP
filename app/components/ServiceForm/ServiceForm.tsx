...
export default function ServiceForm({serviceTitle, selectedDate, endDate, clientIdentifier}: ServiceFormProps){
    ...
    return (
        <div className="w-full h-screen md:h-screen flex flex-col md:flex-row bg-white overflow-hidden">
            ...
            <div className="w-full md:w-[380px] flex-none bg-[#FDFCF7]/50 p-5 md:p-12 border-b md:border-b-0 md:border-r border-gray-100 flex flex-row md:flex-col justify-between items-center md:items-start">
            ...
            <div className="flex-1 h-full overflow-y-auto bg-white">
                <div className="h-full w-full max-w-md mx-auto p-6 md:p-16 flex flex-col">
                    <form action={async (formData: FormData) => {
                    ...
                    }} className="flex-1 flex flex-col justify-between space-y-6 md:space-y-10">
                        ...
                        <div className="space-y-4 md:space-y-8">
                                ...
                                .map((input) => (
                                    <div key={input.name} className="relative group">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-transparent border-b border-gray-100 text-black focus:border-black py-1 md:py-3 outline-none transition-all text-sm md:text-lg font-bold"
                                        />
                                    </div>
                                ))}

                            <div className="pt-2">
                                <label className="text-[7px] md:text-[10px] font-black text-black uppercase tracking-[0.3em] block opacity-30 mb-2">
                                    Evidences
                                </label>
                                <div className="p-4 md:p-6 border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50/30">
                                    <UploadButton
                                        ...
                                        appearance={{
                                            button: "bg-black text-white font-black text-[8px] md:text-[10px] uppercase tracking-widest px-4 py-2 rounded-lg w-full",
                            ...
                            <div className="pb-6 md:pb-0">
                                <button
                                    type="submit"
                                    disabled={isPending}
                                    className={`w-full text-white font-black py-4 md:py-7 rounded-xl md:rounded-[2rem] uppercase tracking-[0.3em] text-[10px] md:text-[12px] transition-all ${
                                        isPending ? 'bg-gray-200' : 'bg-black hover:bg-orange-600'
                                    }`}
                                >
                        ...
                        {error && (
                            <p className="text-[8px] font-bold text-red-500 uppercase text-center">
                                {error}
                            </p>
                        )}
